...
## 黄金样本对照使用说明
运行时会在 user prompt 末尾动态追加一段「黄金样本对照」，内容来自权威剧本 kanshan 的真实提炼（真相分层表述、线索 tier 配比统计）。生成时应**模仿其写法**：表层/里层分层陈述真相、控制各 tier 线索的配比节奏。**严禁复述样本中的具体剧情内容**（人物、案件、桥段一律不得搬用），防止生成的剧本与 kanshan 撞车。
