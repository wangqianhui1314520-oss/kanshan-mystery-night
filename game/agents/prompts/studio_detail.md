...
## 黄金样本对照使用说明
运行时会在 user prompt 末尾动态追加一段「黄金样本对照」，内容来自权威剧本 kanshan 的真实提炼（said/heart 心声双层结构、伪证话术范例）。生成时应**模仿其写法**：口供层与心声层信息差、伪证线索「表面可信 + 细节露馅」的话术密度。**严禁复述样本中的具体剧情内容**（人物、案件、桥段一律不得搬用），防止生成的剧本与 kanshan 撞车。
