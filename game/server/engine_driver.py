...
class EngineDriver:
    ...
    def apply_action(self, session: dict, action_type: str, actor: str,
        ...
        if action_type not in ("search", "chat", "skill", "counsel", "vote", "advance"):
        ...
        if not self.sm.can(action_type):
            ...
            if not finale_skill and not (action_type == "skill" and _skill_name(payload) in ICEBREAKER_SKILLS):
        ...
        if action_type == "skill":
            ...
            if skill == "bid_headline":
                ...
                if payload.get("post") not in self.of._posts:
                    return [], "请选择一条当前可见热搜"
    ...
    def _do_skill(self, session: dict, actor: str, payload: dict) -> list[dict]:
        ...
        if skill == "bid_headline":
            ...
            bid = self.of.bid_headline(actor, faction, payload["amount"], post_id=payload["post"])
    ...
    def _do_vote(self, session: dict, actor: str, payload: dict) -> list[dict]:
        ...
        events = [make_event("vote", sid, rnd, actor, {
            ...
            "coverage": {"pct": 0},
            "source": "engine"})]
        if _ai_speaker_char(actor):
            return events  # 空席先记票，终局只由真人票触发
        if len(self.votes) < needed:
            return events  # 等其余玩家投票
        ...
        for voter, t in self.votes.items():
            # AI 空席票只作回放记录，不入终局多数决（G11）：
            # party 的空席跟票以 PartyBoard 为准（下方合并）；
            # solo 全桌皆空席，若计其票，玩家唯一真人票会被直接淹没。
